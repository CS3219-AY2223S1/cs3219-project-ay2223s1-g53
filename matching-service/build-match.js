...
async function buildMatch(socket) {
  socket.on('match', async (match) => {
    try {
      ...
      const existing = await matchOrm.getExistingMatchByDifficulty(incomingDifficulty, incomingSocketId)
      if (existing) {
        ...
        await matchOrm.createExistingMatch(incomingSocketId, incomingDifficulty);
